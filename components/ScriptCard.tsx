...
export default function ScriptCard({ script, isAuthenticated, currentUserId }: ScriptCardProps) {
  ...
  return (
    <div className="border rounded-lg p-6 bg-white shadow-sm flex flex-col h-[500px]" data-script-id={script.id}>
      <div className="mb-4">
        ...
        {isEditing ? (
          <div className="flex-grow flex flex-col">
            <textarea
              ...
              className="flex-grow w-full p-4 border rounded-lg font-mono text-sm bg-gray-50 resize-none"
            />
          ...
          <Link href={`/scripts/${script.id}`} className="block flex-grow">
            <div className="bg-gray-50 rounded overflow-hidden h-full">
              <Highlight
                theme={themes.vsDark}
              ...
              >
                {({ className, style, tokens, getLineProps, getTokenProps }) => (
                  <pre className={`${className} p-4 h-full`} style={{ ...style, margin: 0 }}>
                    {tokens.map((line, i) => (
